...
class AircraftList(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if (request.GET.get('serial') == '' and request.GET.get('tail') == '') or request.GET == False:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                aircraft_future = [executor.submit(self.get_aircraft_list, request, page) for page in range(1, count)]
                ok = concurrent.futures.wait(aircraft_future)
                for data in ok.done:
                    data_list += data.result()
                print(f'Dataa list - {len(data_list)}')
    ...
    def get_aircraft_list(self, request, page):
        ...
        if request.GET.get('serial') != '' and request.GET.get('tail') != '':
            ...
            params = {'features': True, 'images': True, 'page': page,
                      'search_tail_number': query_tail,
    ...
    def get_page_count(self, request):
        try:
            ...
            query_serial = request.GET.get('serial')
            query_tail = request.GET.get('tail')
            params = {'features': True, 'images': True, 'ordering': 'serial_number',
                      'search_tail_number': query_tail,
                      'search_serial_number': query_serial,
                      }
            response = requests.get(url, headers=self.HEADERS, params=params)
